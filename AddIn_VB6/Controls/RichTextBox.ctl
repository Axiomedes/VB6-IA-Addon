...
Public Event SelChange(ByVal SelType As Integer, _
                       ByVal SelStart As Long, _
                       ByVal SelEnd As Long)
#If VBA7 Then

  Public Event LinkEvent(ByVal wMsg As Long, _
                         ByVal wParam As LongPtr, _
                         ByVal lParam As LongPtr, _
                         ByVal LinkStart As Long, _
                         ByVal LinkEnd As Long)
#Else
...
Public Event DropFiles(ByRef FileList As Variant, _
                       ByVal X As Single, _
                       ByVal Y As Single, _
                       ByVal CharPos As Long, _
                       ByVal Protected As Boolean, _
                       ByRef Cancel As Boolean)

Public Event ModifyProtected(ByRef Allow As Boolean, _
                             ByVal SelStart As Long, _
                             ByVal SelEnd As Long)
...
Public Event OLEGetDropEffect(ByRef Effect As Long, _
...
#If VBA7 Then
  ...
  Public Event OLEGetContextMenu(ByVal SelType As Integer, _
                                 ByVal LpOleObject As LongPtr, _
                                 ByVal SelStart As Long, _
                                 ByVal SelEnd As Long, _
                                 ByRef hMenu As LongPtr)
#Else

  Public Event OLEGetContextMenu(ByVal SelType As Integer, _
                                 ByVal LpOleObject As Long, _
                                 ByVal SelStart As Long, _
                                 ByVal SelEnd As Long, _
                                 ByRef hMenu As Long)
#End If
...
#Else
  ...
  Private Declare Function CreateWindowEx _
                  Lib "user32" _
                  Alias "CreateWindowExW" (ByVal dwExStyle As Long, _
  ...
  Private Declare Function LoadCursor _
                  Lib "user32" _
                  Alias "LoadCursorW" (ByVal hInstance As Long, _
  ...
  Private Declare Function StgCreateDocFile _
                  Lib "ole32" _
                  Alias "StgCreateDocfile" (ByVal pwcsName As Long, _
                                            ByVal grfMode As Long, _
                                            ByVal Reserved As Long, _
                                            ByRef ppStgOpen As OLEGuids.IStorage) As Long

  Private Declare Function OleSetContainedObject _
                  Lib "ole32" (ByVal pUnknown As IUnknown, _
                               ByVal fContained As Long) As Long

  Private Declare Function OleCreateFromFile _
                  Lib "ole32" (ByRef pCLSID As Any, _
                               ByVal lpszFileName As Long, _
                               ByRef riid As Any, _
                               ByVal RenderOpt As Long, _
                               ByVal lpFormatEtc As Long, _
                               ByVal pClientSite As OLEGuids.IOleClientSite, _
                               ByVal pStg As OLEGuids.IStorage, _
                               ByRef ppvObj As OLEGuids.IOleObject) As Long

  Private Declare Function OleCreateLinkToFile _
                  Lib "ole32" (ByVal lpszFileName As Long, _
                               ByRef riid As Any, _
                               ByVal RenderOpt As Long, _
                               ByVal lpFormatEtc As Long, _
                               ByVal pClientSite As OLEGuids.IOleClientSite, _
  ...
  Private Declare Function OleCreateStaticFromData _
                  Lib "ole32" (ByVal pSrcDataObject As OLEGuids.IDataObject, _
                               ByRef riid As Any, _
                               ByVal RenderOpt As Long, _
                               ByVal lpFormatEtc As Long, _
                               ByVal pClientSite As OLEGuids.IOleClientSite, _
  ...
  Private Declare Function RegisterClipboardFormat _
                  Lib "user32" _
                  Alias "RegisterClipboardFormatW" (ByVal lpString As Long) As Long

  Private Declare Function SHCreateDataObject _
                  Lib "shell32" (ByVal pIDLFolder As Long, _
                                 ByVal cIDL As Long, _
                                 ByVal apIDL As LongPtr, _
                                 ByRef pDataInner As Any, _
                                 ByRef riid As Any, _
                                 ByRef ppDataObject As OLEGuids.IDataObject) As Long

  Private Declare Function SHCreateFileDataObject _
                  Lib "shell32" _
                  Alias "#740" (ByVal pIDLFolder As LongPtr, _
                                ByVal cIDL As Long, _
                                ByVal apIDL As LongPtr, _
                                ByRef pDataInner As Any, _
                                ByRef ppDataObject As OLEGuids.IDataObject) As Long
  ...
  Private Declare Function LoadLibrary _
                  Lib "kernel32" _
                  Alias "LoadLibraryW" (ByVal lpLibFileName As Long) As Long

  Private Declare Function FreeLibrary Lib "kernel32" (ByVal hLibModule As Long) As Long

  Private Declare Function GetProcAddress _
                  Lib "kernel32" (ByVal hModule As Long, _
                                  ByVal lpProcName As Any) As Long

  Private Declare Function DragDetect _
                  Lib "user32" (ByVal hWnd As Long, _
                                ByVal XY As Currency) As Long
  ...
  Private Declare Function DragQueryPoint _
                  Lib "shell32" (ByVal hDrop As Long, _
                                 ByRef lpPoint As POINTAPI) As Long
  ...
  Private Declare Function InsertMenuItem _
                  Lib "user32" _
                  Alias "InsertMenuItemW" (ByVal hMenu As Long, _
  ...
  Private Declare Function OleUIPasteSpecial _
                  Lib "oledlg" _
                  Alias "OleUIPasteSpecialW" (ByRef pOleUIPasteSpecial As TOLEUIPASTESPECIAL) As Long
#End If
#If ImplementThemedBorder = True Then
  ...
  #Else
    ...
    Private Declare Function OpenThemeData _
                    Lib "uxtheme" (ByVal hWnd As Long, _
                                   ByVal lpszClassList As Long) As Long

    Private Declare Function CloseThemeData Lib "uxtheme" (ByVal Theme As Long) As Long

    Private Declare Function IsThemeBackgroundPartiallyTransparent _
                    Lib "uxtheme" (ByVal Theme As Long, _
    ...
    Private Declare Function GetDCEx _
                    Lib "user32" (ByVal hWnd As Long, _
...
Private Sub IObjectSafety_GetInterfaceSafetyOptions(ByRef riid As OLEGuids.OLECLSID, _
...
Private Sub IObjectSafety_SetInterfaceSafetyOptions(ByRef riid As OLEGuids.OLECLSID, _
                                                    ByVal dwOptionsSetMask As Long, _
                                                    ByVal dwEnabledOptions As Long)
End Sub

#If VBA7 Then
  Private Sub IOleInPlaceActiveObjectVB_TranslateAccelerator(ByRef Handled As Boolean, _
...
#If VBA7 Then
  Private Sub IOleControlVB_GetControlInfo(ByRef Handled As Boolean, _
                                           ByRef AccelCount As Integer, _
                                           ByRef AccelTable As LongPtr, _
                                           ByRef Flags As Long)
  #Else
...
#If VBA7 Then
  Private Sub IOleControlVB_OnMnemonic(ByRef Handled As Boolean, _
                                       ByVal hWnd As LongPtr, _
                                       ByVal wMsg As Long, _
                                       ByVal wParam As LongPtr, _
                                       ByVal lParam As LongPtr, _
...
Private Sub IPerPropertyBrowsingVB_GetDisplayString(ByRef Handled As Boolean, _
...
Private Sub IPerPropertyBrowsingVB_GetPredefinedStrings(ByRef Handled As Boolean, _
                                                        ByVal DispId As Long, _
                                                        ByRef StringsOut() As String, _
                                                        ByRef CookiesOut() As Long)
...
Private Sub IPerPropertyBrowsingVB_GetPredefinedValue(ByRef Handled As Boolean, _
                                                      ByVal DispId As Long, _
...
Public Function Find(ByVal Text As String, _
                     Optional ByVal Min As Long, _
                     Optional ByVal Max As Long = -1, _
                     Optional ByVal Options As RtfFindOptionConstants) As Long
...
Public Sub Span(ByVal CharacterSet As String, _
                Optional ByVal Forward As Boolean, _
...
Public Sub UpTo(ByVal CharacterSet As String, _
                Optional ByVal Forward As Boolean, _
                Optional ByVal Negate As Boolean)
...
#If VBA7 Then
  Public Sub SelPrint(ByVal hDC As LongPtr, _
                      Optional ByVal CallStartEndDoc As Boolean = True, _
                      Optional ByVal DocName As String = "RICHTEXT", _
                      Optional ByVal LeftMargin As Long, _
                      Optional ByVal TopMargin As Long, _
                      Optional ByVal RightMargin As Long, _
                      Optional ByVal BottomMargin As Long)
  #Else
...
#If VBA7 Then
  Public Sub PrintDoc(ByVal hDC As LongPtr, _
                      Optional ByVal CallStartEndDoc As Boolean = True, _
                      Optional ByVal DocName As String = "RICHTEXT", _
                      Optional ByVal LeftMargin As Long, _
                      Optional ByVal TopMargin As Long, _
...
Public Sub SaveFile(ByVal FileName As String, _
                    Optional ByVal Format As RtfLoadSaveFormatConstants = RtfLoadSaveFormatRTF, _
                    Optional ByVal SelectionOnly As Boolean)
...
Public Sub LoadFile(ByVal FileName As String, _
                    Optional ByVal Format As RtfLoadSaveFormatConstants = RtfLoadSaveFormatRTF, _
                    Optional ByVal SelectionOnly As Boolean)
...
Public Sub OLEObjectsAddFromFile(ByVal FileName As String, _
                                 Optional ByVal LinkToFile As Boolean)
...
Public Sub OLEObjectsAddFromPicture(ByVal Picture As IPictureDisp, _
                                    Optional ByVal ClipFormat As Variant)
...
#If VBA7 Then
  Public Function OLEObjectsGet(ByVal IndexObj As Long, _
                                Optional ByVal CharPos As Long) As LongPtr
  #Else
...
Private Sub CreatePrintJob(ByVal Min As Long, _
                           ByVal Max As Long, _
                           ByVal Length As Long, _
                           ByVal hDC As LongPtr, _
                           ByVal CallStartEndDoc As Boolean, _
                           ByVal DocName As String, _
                           ByVal LeftMargin As Long, _
                           ByVal TopMargin As Long, _
                           ByVal RightMargin As Long, _
                           ByVal BottomMargin As Long)
...
Private Function ShowPasteSpecialDlg(ByRef wFormat As Long) As Boolean
  ...
  Select Case LangID
    ...
    Case &H8 ' Greek
      szFormat(0) = ChrW(&H39C&) & ChrW(&H3BF&) & ChrW(&H3C1&) & ChrW(&H3C6&) & ChrW(&H3BF&) & ChrW(&H3C0&) & ChrW(&H3BF&) & ChrW(&H3B9&) & ChrW(&H3B7&) & ChrW(&H3BC&) & ChrW(&H3AD&) & ChrW(&H3BD&) & ChrW(&H3BF&) & " " & ChrW(&H3BA&) & ChrW(&H3B5&) & ChrW(&H3AF&) & ChrW(&H3BC&) & ChrW(&H3B5&) & ChrW(&H3BD&) & ChrW(&H3BF&) & " (RTF)"
      szFormat(1) = ChrW(&H39C&) & ChrW(&H3B7&) & " " & ChrW(&H3BC&) & ChrW(&H3BF&) & ChrW(&H3C1&) & ChrW(&H3C6&) & ChrW(&H3BF&) & ChrW(&H3C0&) & ChrW(&H3BF&) & ChrW(&H3B9&) & ChrW(&H3B7&) & ChrW(&H3BC&) & ChrW(&H3AD&) & ChrW(&H3BD&) & ChrW(&H3BF&) & " " & ChrW(&H3BA&) & ChrW(&H3B5&) & ChrW(&H3AF&) & ChrW(&H3BC&) & ChrW(&H3B5&) & ChrW(&H3BD&) & ChrW(&H3BF&)
      szFormat(2) = ChrW(&H39A&) & ChrW(&H3B5&) & ChrW(&H3AF&) & ChrW(&H3BC&) & ChrW(&H3B5&) & ChrW(&H3BD&) & ChrW(&H3BF&) & " Unicode " & ChrW(&H3C7&) & ChrW(&H3C9&) & ChrW(&H3C1&) & ChrW(&H3AF&) & ChrW(&H3C2&) & " " & ChrW(&H3BC&) & ChrW(&H3BF&) & ChrW(&H3C1&) & ChrW(&H3C6&) & ChrW(&H3BF&) & ChrW(&H3C0&) & ChrW(&H3BF&) & ChrW(&H3AF&) & ChrW(&H3B7&) & ChrW(&H3C3&) & ChrW(&H3B7&)
      ...
      szResult(0) = ChrW(&H3BA&) & ChrW(&H3B5&) & ChrW(&H3AF&) & ChrW(&H3BC&) & ChrW(&H3B5&) & ChrW(&H3BD&) & ChrW(&H3BF&) & " " & ChrW(&H3BC&) & ChrW(&H3B5&) & " " & ChrW(&H3B3&) & ChrW(&H3C1&) & ChrW(&H3B1&) & ChrW(&H3BC&) & ChrW(&H3BC&) & ChrW(&H3B1&) & ChrW(&H3C4&) & ChrW(&H3BF&) & ChrW(&H3C3&) & ChrW(&H3B5&) & ChrW(&H3B9&) & ChrW(&H3C1&) & ChrW(&H3AC&) & " " & ChrW(&H3BA&) & ChrW(&H3B1&) & ChrW(&H3B9&) & " " & ChrW(&H3BC&) & ChrW(&H3BF&) & ChrW(&H3C1&) & ChrW(&H3C6&) & ChrW(&H3BF&) & ChrW(&H3C0&) & ChrW(&H3BF&) & ChrW(&H3AF&) & ChrW(&H3B7&) & ChrW(&H3C3&) & ChrW(&H3B7&) & " " & ChrW(&H3C0&) & ChrW(&H3AF&) & ChrW(&H3BD&) & ChrW(&H3B1&) & ChrW(&H3BA&) & ChrW(&H3B1&)
      szResult(1) = ChrW(&H3BA&) & ChrW(&H3B5&) & ChrW(&H3AF&) & ChrW(&H3BC&) & ChrW(&H3B5&) & ChrW(&H3BD&) & ChrW(&H3BF&) & " " & ChrW(&H3C7&) & ChrW(&H3C9&) & ChrW(&H3C1&) & ChrW(&H3AF&) & ChrW(&H3C2&) & " " & ChrW(&H3BA&) & ChrW(&H3B1&) & ChrW(&H3BC&) & ChrW(&H3AF&) & ChrW(&H3B1&) & " " & ChrW(&H3BC&) & ChrW(&H3BF&) & ChrW(&H3C1&) & ChrW(&H3C6&) & ChrW(&H3BF&) & ChrW(&H3C0&) & ChrW(&H3BF&) & ChrW(&H3AF&) & ChrW(&H3B7&) & ChrW(&H3C3&) & ChrW(&H3B7&)
      szResult(2) = szResult(1)
    ...
    Case &H19 ' Russian
      szFormat(0) = ChrW(&H424&) & ChrW(&H43E&) & ChrW(&H440&) & ChrW(&H43C&) & ChrW(&H430&) & ChrW(&H442&) & ChrW(&H438&) & ChrW(&H440&) & ChrW(&H43E&) & ChrW(&H432&) & ChrW(&H430&) & ChrW(&H43D&) & ChrW(&H43D&) & ChrW(&H44B&) & ChrW(&H439&) & " " & ChrW(&H442&) & ChrW(&H435&) & ChrW(&H43A&) & ChrW(&H441&) & ChrW(&H442&) & " (RTF)"
      szFormat(1) = ChrW(&H41D&) & ChrW(&H435&) & ChrW(&H444&) & ChrW(&H43E&) & ChrW(&H440&) & ChrW(&H43C&) & ChrW(&H430&) & ChrW(&H442&) & ChrW(&H438&) & ChrW(&H440&) & ChrW(&H43E&) & ChrW(&H432&) & ChrW(&H430&) & ChrW(&H43D&) & ChrW(&H43D&) & ChrW(&H44B&) & ChrW(&H439&) & " " & ChrW(&H442&) & ChrW(&H435&) & ChrW(&H43A&) & ChrW(&H441&) & ChrW(&H442&)
      szFormat(2) = ChrW(&H41D&) & ChrW(&H435&) & ChrW(&H444&) & ChrW(&H43E&) & ChrW(&H440&) & ChrW(&H43C&) & ChrW(&H430&) & ChrW(&H442&) & ChrW(&H438&) & ChrW(&H440&) & ChrW(&H43E&) & ChrW(&H432&) & ChrW(&H430&) & ChrW(&H43D&) & ChrW(&H43D&) & ChrW(&H44B&) & ChrW(&H439&) & " " & ChrW(&H442&) & ChrW(&H435&) & ChrW(&H43A&) & ChrW(&H441&) & ChrW(&H442&) & " Unicode"
      ...
      szResult(0) = ChrW(&H442&) & ChrW(&H435&) & ChrW(&H43A&) & ChrW(&H441&) & ChrW(&H442&) & " " & ChrW(&H441&) & ChrW(&H43E&) & " " & ChrW(&H448&) & ChrW(&H440&) & ChrW(&H438&) & ChrW(&H444&) & ChrW(&H442&) & ChrW(&H43E&) & ChrW(&H43C&) & " " & ChrW(&H438&) & " " & ChrW(&H444&) & ChrW(&H43E&) & ChrW(&H440&) & ChrW(&H43C&) & ChrW(&H430&) & ChrW(&H442&) & ChrW(&H438&) & ChrW(&H440&) & ChrW(&H43E&) & ChrW(&H432&) & ChrW(&H430&) & ChrW(&H43D&) & ChrW(&H438&) & ChrW(&H435&) & ChrW(&H43C&) & " " & ChrW(&H442&) & ChrW(&H430&) & ChrW(&H431&) & ChrW(&H43B&) & ChrW(&H438&) & ChrW(&H446&) & ChrW(&H44B&)
      szResult(1) = ChrW(&H442&) & ChrW(&H435&) & ChrW(&H43A&) & ChrW(&H441&) & ChrW(&H442&) & " " & ChrW(&H431&) & ChrW(&H435&) & ChrW(&H437&) & " " & ChrW(&H444&) & ChrW(&H43E&) & ChrW(&H440&) & ChrW(&H43C&) & ChrW(&H430&) & ChrW(&H442&) & ChrW(&H438&) & ChrW(&H440&) & ChrW(&H43E&) & ChrW(&H432&) & ChrW(&H430&) & ChrW(&H43D&) & ChrW(&H438&) & ChrW(&H44F&)
      szResult(2) = szResult(1)
...
#End If
#If VBA7 Then
  Private Function ISubclass_Message(ByVal hWnd As LongPtr, _
                                     ByVal wMsg As Long, _
                                     ByVal wParam As LongPtr, _
                                     ByVal lParam As LongPtr, _
                                     ByVal dwRefData As LongPtr) As LongPtr
  #Else
...
Friend Sub FIRichEditOleCallback_GetNewStorage(ByRef RetVal As Long, _
                                               ByRef ppStorage As OLEGuids.IStorage)
...
Friend Sub FIRichEditOleCallback_GetDragDropEffect(ByVal Drag As Boolean, _
                                                   ByVal KeyState As Long, _
                                                   ByRef dwEffect As Long)
...
#If VBA7 Then
  Friend Sub FIRichEditOleCallback_GetContextMenu(ByVal SelType As Integer, _
                                                  ByVal LpOleObject As LongPtr, _
                                                  ByVal lpCharRange As LongPtr, _
                                                  ByRef hMenu As LongPtr)
  #Else
    ...
    Else
      ...
      For i = 1 To 8
        ...
        Select Case LangID
          ...
          Case &H4 ' Chinese
            Text = VBA.Choose(i, ChrW(&H64A4&) & ChrW(&H6D88&) & "(&U)" & vbTab & "Ctrl+Z", ChrW(&H6062&) & ChrW(&H590D&) & "(&R)" & vbTab & "Ctrl+Y", ChrW(&H526A&) & ChrW(&H5207&) & "(&T)" & vbTab & "Ctrl+X", ChrW(&H590D&) & ChrW(&H5236&) & "(&C)" & vbTab & "Ctrl+C", ChrW(&H7C98&) & ChrW(&H8D34&) & "(&P)" & vbTab & "Ctrl+V", ChrW(&H7C98&) & ChrW(&H8D34&) & ChrW(&H7EAF&) & ChrW(&H6587&) & ChrW(&H672C&) & vbTab & "Ctrl+Shift+V", ChrW(&H9009&) & ChrW(&H62E9&) & ChrW(&H6027&) & ChrW(&H7C98&) & ChrW(&H8D34&) & vbTab & "Ctrl+Alt+V", ChrW(&H5220&) & ChrW(&H9664&) & "(&D)" & vbTab & "Del")
          ...
          Case &H11 ' Japanese
            Text = VBA.Choose(i, ChrW(&H5143&) & ChrW(&H306B&) & ChrW(&H623B&) & ChrW(&H3059&) & "(&U)" & vbTab & "Ctrl+Z", ChrW(&H3084&) & ChrW(&H308A&) & ChrW(&H76F4&) & ChrW(&H3057&) & "(&R)" & vbTab & "Ctrl+Y", ChrW(&H5207&) & ChrW(&H308A&) & ChrW(&H53D6&) & ChrW(&H308A&) & "(&T)" & vbTab & "Ctrl+X", ChrW(&H30B3&) & ChrW(&H30D4&) & ChrW(&H30FC&) & "(&C)" & vbTab & "Ctrl+C", ChrW(&H8CBC&) & ChrW(&H308A&) & ChrW(&H4ED8&) & ChrW(&H3051&) & "(&P)" & vbTab & "Ctrl+V", ChrW(&H30D7&) & ChrW(&H30EC&) & ChrW(&H30FC&) & ChrW(&H30F3&) & " " & ChrW(&H30C6&) & ChrW(&H30AD&) & ChrW(&H30B9&) & ChrW(&H30C8&) & ChrW(&H3068&) & ChrW(&H3057&) & ChrW(&H3066&) & ChrW(&H8CBC&) & ChrW(&H308A&) & ChrW(&H4ED8&) & ChrW(&H3051&) & ChrW(&H308B&) & vbTab & "Ctrl+Shift+V", ChrW(&H5F62&) & ChrW(&H5F0F&) & ChrW(&H3092&) & ChrW(&H9078&) & ChrW(&H629E&) & ChrW(&H3057&) & ChrW(&H3066&) & ChrW(&H8CBC&) & ChrW(&H308A&) & ChrW(&H4ED8&) & ChrW(&H3051&) & vbTab & "Ctrl+Alt+V", ChrW(&H524A&) & ChrW(&H9664&) & "(&D)" & vbTab & "Del")
...
Private Function WindowProcControl(ByVal hWnd As LongPtr, _
                                   ByVal wMsg As Long, _
                                   ByVal wParam As LongPtr, _
...
Private Function WindowProcUserControl(ByVal hWnd As LongPtr, _
                                       ByVal wMsg As Long, _
                                       ByVal wParam As LongPtr, _
                                       ByVal lParam As LongPtr) As LongPtr
